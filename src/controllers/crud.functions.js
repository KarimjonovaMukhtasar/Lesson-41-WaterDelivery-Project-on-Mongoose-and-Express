export function findAll(modelName){
    ...
    const fields = Object.keys(model.schema.paths).filter((f)=> !["_id", "__v", "createdAt", "updatedAt"].includes(f));
    const query = search 
    ? {
        $or: fields.map((field)=> ({
...
export function findOne(modelName){
    return async(req,res,next) => {
         try{
    const model = modelName
    if(!model){
...
export function deletee(modelName){
    return async (req,res,next) =>{
        try{
    const model = modelName
    if(!model){
